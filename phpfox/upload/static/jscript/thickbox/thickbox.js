...
$Behavior.addDraggableToBoxes = function()
{
    ...
    {
    	var bReturn = tb_show('', this.href, this);
...
function tb_show(caption, url, thisObject, sForceMessage, bForceNoCilck) 
{
	...
	if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp')
	{
		...
		imgPreloader.onload = function()
		{
			...
			sHtml += '</div><div class="js_box_close"><a href="#" onclick="return js_box_remove(this);">Close</a></div></div></div>';			
	...
	if ($(thisObject).hasClass('photo_holder_image') && !empty($(thisObject).attr('rel')))
	{
		...
		var queryString = '' + getParam('sGlobalTokenName') + '[call]=photo.view&width=940&req2=' + $(thisObject).attr('rel') + '&theater=true&no_remove_box=true';
	...
	if (!bIsAlreadyOpen)
	{
		...
		sHtml += '<div class="js_box_content"><span class="js_box_loader">Loading...</span></div>';
		// if (!bIsPhotoImage)
		{
			sHtml += '<div class="js_box_close"><a href="#" onclick="return js_box_remove(this);">Close</a><span class="js_box_history">' + params[getParam('sGlobalTokenName') + '[call]'] + '</span></div>';
		}
		sHtml += '</div>';
