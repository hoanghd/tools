 ...
 * @author			Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: template.class.php 3356 2011-10-25 14:38:13Z Miguel_Espinoza $
 */
...
{	
	...
	{
		$sVersion = md5((((defined('PHPFOX_NO_CSS_CACHE') && PHPFOX_NO_CSS_CACHE) || $this->_bIsTestMode === true) ? PHPFOX_TIME : PhpFox::getId() . Phpfox::getBuild()) . (defined('PHPFOX_INSTALLER') ? '' : '-' . Phpfox::getParam('core.css_edit_id') . '-' . $this->_sThemeFolder . '-' . $this->_sStyleFolder));	
...
//					'core.toggle_fullscreen',	
			...
			if (!$this->_isCached($sFile))
			{
				...
				{				
					...
					$aSubTemplate = $oDb->select('html_data')
						->from(Phpfox::getT('theme_template'))
						->where("folder = '" . self::$_sStaticThemeFolder . "' AND type_id = '" . $oDb->escape($aMatches[4]) . "' AND module_id = '" . $oDb->escape($aMatches[2]) . "' AND name = '" . $oDb->escape(str_replace('_', '/', $aMatches[5])) . "'")
						->execute('getSlaveRow');
				}
	...
	public function getMenu($sConnection = null)
	{
		...
		$sCachedId = $oCache->set(array('theme', 'menu_' . str_replace(array('/', '\\'), '_', $sConnection)));
