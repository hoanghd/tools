 ...
 * @package 		Phpfox
 * @subpackage 		Template
 * @version 		$Id: cache.class.php 3342 2011-10-21 12:59:32Z Raymond_Benc $
 */
		...
		switch ($sFunction)
		{
					...
					case 'select':
						...
						$sContent .= "\n".' && is_numeric(\''.$aArgs["id"].'\') && in_array(\''.$aArgs["id"].'\', $this->_aVars[\'aForms\']'. (isset($aArgs['parent']) ? '[\''.$aArgs["parent"].'\']' : '') .'))
							...
							{'."\n".'
								...
								{
									echo ' . (isset($aArgs['selected']) ? '" selected=\"selected\""' : '""') . ';
								}
							}
							?>' . "\n";
						break;
			...
			case 'select_date':
				...
				if ($bUseJquery)
				{
					...
					$sReturn = '<div class="js_datepicker_core"><div class="js_datepicker_image"></div><span class="js_datepicker_holder"><div style="display:none;">' . $sReturn . '</div><input type="text" name="js_' . $sPrefix . '_datepicker" value="' . $sValue . '" class="js_date_picker" /></span> ';
				}
				
				if (isset($aArgs['add_time']))
				{
					$sReturn .= Phpfox::getPhrase($this->_removeQuote($aArgs['time_separator']));
					$aHours = range(0, 23);
