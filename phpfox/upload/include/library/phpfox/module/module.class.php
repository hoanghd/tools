 ...
 * @author			Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: module.class.php 3342 2011-10-21 12:59:32Z Raymond_Benc $
 */
class Phpfox_Module
{	
	/**
	...
	{	
		...
		{
			if (in_array($this->getFullControllerName(), Phpfox::getService('core')->getSecurePages()))
			{
				if (!isset($_SERVER['HTTPS']))
				{
					Phpfox::getLib('url')->send($this->getFullControllerName());
				}
			}
		}
	}	
	...
	{		
		...
		$iBlockCacheId = $oCache->set(array('block', 'all'));
		$sLocationCacheId = $oCache->set(array('block', 'location'));
		$sMoveBlockId = $oCache->set(array('block', 'move'));
		$sSourceCodeBlockId = $oCache->set(array('block', 'source_code'));
		...
		{			
			...
			//d($aRows); die();
	/*		
			// get the user defined order of blocks here
			if (Phpfox::getParam('profile.can_drag_drop_blocks_on_profile') && Phpfox::getUserParam('profile.can_custom_design_own_profile'))
			{
				$aCustomOrder = Phpfox::getLib('database')->select('b.block_id, ud.cache_id, ud.ordering')
					->from(Phpfox::getT('user_dashboard'), 'ud')
					->join(Phpfox::getT('block'), 'b', 'b.block_id = ud.block_id')
					->where('user_id = ' . Phpfox::getUserId())
					->order('ud.ordering DESC')					
					->execute('getSlaveRows');				
				d($aCustomOrder);
				foreach ($aRows as $iPosition => $aBlocks)
				{
					foreach ($aBlocks as $iBlockPlace => $aBlock)
					{
						if ($aBlock['m_connection'] != 'core.index-member')
						{
							continue;
						}
						
						foreach ($aCustomOrder as $aCustom)
						{
							if ($aBlock['block_id'] == $aCustom['block_id'])
							{
								d($aRows[$iPosition]);
								d('============> Changing order');
								unset($aRows[$iPosition][$iBlockPlace]);
								unset($aBlocks[$iBlockPlace]);
								$aRows[$iPosition] = array_merge($aBlock, $aBlocks);
								d($aRows[$iPosition]);
