 ...
 * @version 		$Id: custom.class.php 3296 2011-10-12 13:29:57Z Raymond_Benc $
 */
	...
	public function searchFields($aParams)
	{
		...
		foreach ($aParams as $iKey => $mValue)
		{
			...
			if ($aFields[$iKey]['var_type'] == 'select'
			    || $aFields[$iKey]['var_type'] == 'multiselect'
				...
				)
			{
				...
				if (is_array($mValue))
				{
				    $mValue = array_pop($mValue);
				}
				if ( (int)$mValue == 0 ) // Any
				{
				    $aRows = $this->database()->select('option_id')
					...
					$aConds[] = array(
					    ...
					    'alias' => 'mcv',
					    'on' => 'mcv.user_id = u.user_id',
					    'where' => ' mcv.option_id = ' . $aRow['option_id']);
				    }
				...
				$aConds[] = array(
				    ...
				    'alias' => 'mcv',
				    'on' => 'mcv.user_id = u.user_id',
				    'where' => ' mcv.option_id = ' . (int) $mValue);
			}
			// a different case because checkboxes dont allow input "Any" as one option
			elseif ($aFields[$iKey]['var_type'] == 'checkbox')
			{
			    foreach ($mValue as $iOptionId => $iOption)
			    {
				$aConds[] = array(
				    'table' => Phpfox::getT('user_custom_multiple_value'),
				    'alias' => 'mcv',
				    'on' => 'mcv.user_id = u.user_id',
				    'where' => ' mcv.option_id = ' . $iOptionId);
			    }
			}
			else
			{
