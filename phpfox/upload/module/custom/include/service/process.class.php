 ...
 * @version 		$Id: process.class.php 3197 2011-09-27 12:37:51Z Raymond_Benc $
 */
...
{	
	...
	{       
		...
		$this->database()->delete(Phpfox::getT('user_custom_multiple_value'), 'user_id = ' . $iEditUserId . ' AND field_id = ' . $aField['field_id']);
		 ...
		 case 'radio':
			...
			case 'checkbox':
				...
				{					
					foreach ($sOptionValue as $iKey => $aVal)
					{
						...
						/* Add the option this user chose */
						$this->database()->insert(Phpfox::getT('user_custom_multiple_value'), array('user_id' => $iEditUserId, 'field_id' => $aField['field_id'], 'option_id' => $aVal['option_id']));
					}
				}
			...
			case 'textarea':		
				...
				else 
				{
					...
					$aExisting = $this->database()->select('user_id as userCustom')
	...
	private function _addOptions($iFieldId, &$aVals)
	{
		...
		// format so it groups by language		
		foreach ($aVals['option'] as $iKey => $aOptions)
		{
			...
			$sPhraseVar = 'cf_option_' . $iFieldId . '_' . $iKey;
		...
		return true;
			...
			$iCnt = $this->database()->select('COUNT(*)')
				...
				->where('module_id = \'' . $this->database()->escape($aVals['module_id']) . '\' AND var_name LIKE \'%' . $this->database()->escape($sOptionVarName) . '%\'')
				->execute('getField');	
				
			if ($iCnt)
			{
				$sOptionVarName = $sOptionVarName . '_' . ($iCnt + 1);
			}				
			
			$aFeed = array();			
			$sLanguage = array_keys($aOption);
			$sLanguage = array_pop($sLanguage);
			if (!empty($sLanguage) &&
					isset($aVals['feed']) && 
					isset($aVals['feed'][$iKey]) && 
					isset($aVals['feed'][$iKey][$sLanguage]))
			{
				Phpfox::getService('language.phrase.process')->add(array(
					'var_name' => $sOptionVarName . '_feed',					
					'module' => $aVals['module_id'] . '|' . $aVals['module_id'],
					'product_id' => $aVals['product_id'],
					'text' => array($sLanguage => $aVals['feed'][$iKey][$sLanguage])
				));				
			}
