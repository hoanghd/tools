...
class Custom_Component_Controller_Admincp_Relationships extends Phpfox_Component
{
	...
	public function process()
	{
		if (($aVals = $this->request()->getArray('val')))
		{
			...
			{
				$this->url()->send('admincp.custom.relationships',array(), 'Status added');
			}
		}
		...
		if ( ($iId = $this->request()->getInt('delete')))
		{
			...
			{
				$this->url()->send('admincp.custom.relationships',array(), 'Status deleted');
			}
		}
		...
		if ($iEdit = $this->request()->getInt('edit'))
		{
			...
			if (empty($aEdit))
			{
				Phpfox_Error::display('Not found');
			}
			else
			{
