 ...
 * @version 		$Id: index.class.php 3346 2011-10-24 15:20:05Z Raymond_Benc $
 */
class Music_Component_Controller_Index extends Phpfox_Component
{
	...
	{		
		...
		if ($this->request()->get('req2') == 'delete' && ($iDeleteId = $this->request()->getInt('id')) && Phpfox::getService('music.process')->delete($iDeleteId))
		{
		...
		switch ($sView)
		{
			...
			default:
				if ($bIsProfile === true)
				{
					$this->search()->setCondition("AND m.view_id IN(" . ($aUser['user_id'] == Phpfox::getUserId() ? '0,1' : '0') . ") AND m.privacy IN(" . (Phpfox::getParam('core.section_privacy_item_browsing') ? '%PRIVACY%' : Phpfox::getService('privacy')->getForBrowse($aUser)) . ") AND m.user_id = " . $aUser['user_id'] . "");	
				}
				else
				{				
