 ...
 * @version 		$Id: album.class.php 2595 2011-05-09 14:01:09Z Raymond_Benc $
 */
class Music_Component_Controller_Browse_Album extends Phpfox_Component
{
	...
	public function process()
	{
		...
		{
			$this->url()->send('music.browse.album', null, 'Album successfully deleted.');
		}		
		...
		$this->search()->set(array(
				...
				'search_tool' => array(
					'table_alias' => 'm',
					'search' => array(
						'action' => ($bIsProfile === true ? $this->url()->makeUrl($aUser['user_name'], array('music.album', 'view' => $this->request()->get('view'))) : $this->url()->makeUrl('music.album', array('view' => $this->request()->get('view')))),
						'default_value' => 'Search Albums...',
						'name' => 'search',
						'field' => 'm.name'
					),
					'sort' => array(
						'latest' => array('m.time_stamp', 'Latest'),
						'most-viewed' => array('m.total_view', 'Most Viewed'),
						'most-liked' => array('m.total_like', 'Most Liked'),
						'most-talked' => array('m.total_comment', 'Most Discussed')
					),
					'show' => array(10, 20, 30)
				)
			)
