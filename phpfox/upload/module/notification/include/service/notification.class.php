	...
	public function getUsers($aNotification, $bRemove = false)
	{
		if (isset($aNotification['extra_users']) && is_array($aNotification['extra_users']) && count($aNotification['extra_users']))
		{
			...
			{	
				...
				$sStr .= '<span class="drop_data_user">' . ((isset($aUser['user_id']) && $aUser['user_id'] == Phpfox::getUserId()) ? 'You' : $aUser['full_name']) . '</span>, ';
			}
			...
			{	
				if (isset($sExtraTotalUsers))
				{
					...
					{
						$sStr = $sStr . ' and ' . $sExtraTotalUsers . ' other ';
					}
					else 
					{
						$sStr = $sStr . ' and ' . $sExtraTotalUsers . ' others ';
					}					
				}
				else 
				{
			...
			else 
			{
				if (isset($sExtraTotalUsers))
				{
					if ($sExtraTotalUsers === 1)
					{
						$sStr = $sStr . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span> and ' . $sExtraTotalUsers . ' other ';
					}
					else 
					{
						$sStr = $sStr . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span> and ' . $sExtraTotalUsers . ' others ';
					}				
				}
				else 
				{
					$sStr = rtrim($sStr, ', ') . ' and <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span>';
				}
			}
		...
		$sStr = '<span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $aNotification['full_name']) . '</span>';
