 ...
 * @package 		Phpfox
 * @version 		$Id: add.html.php 1955 2010-10-27 14:23:41Z Miguel_Espinoza $
 */
...
<form action="{url link="current"}" method="post" id="frm_announcement">
	...
	<div class="table">
		<div class="table_left">
			{phrase var='announcement.announcement'}:
		</div>
		<div class="table_right">
		{foreach from=$aAnnouncement.language item=aLanguage}
			<b>{$aLanguage.title}</b>
			<div class="p_4">
			    <textarea cols="70" rows="5" name="val[content][{$aLanguage.language_id}][text]">{if isset($aLanguage.content)}{$aLanguage.content}{/if}</textarea>
			    <input type="hidden" name="val[content][{$aLanguage.language_id}][is_default]" value="{$aLanguage.is_default}">
			</div>
		    {/foreach}
			<div class="extra_info">
				{phrase var='announcement.the_announcement_can_be_left_open_if_everything_can_be_added_into_the_intro'}				
			</div>
		</div>
