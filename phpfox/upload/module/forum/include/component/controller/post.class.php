...
class Forum_Component_Controller_Post extends Phpfox_Component
{
	...
	public function process()
	{
		...
		)
		{
			...
			$this->template()->setBreadcrumb('Pages', $this->url()->makeUrl('pages'));
			$this->template()->setBreadcrumb($aCallback['title'], $aCallback['url_home']);
			$this->template()->setBreadcrumb('Discussions', $aCallback['url_home'] . 'forum/');
			...
			{				
				...
				{				
					if ($bIsEdit)
					{
						...
						{
							$this->url()->permalink('forum.thread', $iId, Phpfox::getLib('parse.input')->clean($aVals['title'], 255), true, Phpfox::getPhrase('forum.thread_successfully_updated'));							
						}						
					}
					else 
					{
