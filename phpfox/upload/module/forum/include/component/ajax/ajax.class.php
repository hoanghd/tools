...
class Forum_Component_Ajax_Ajax extends Phpfox_Ajax
{
	...
	public function deletePost()
	{
		...
		{
			if ((Phpfox::getUserParam('forum.can_delete_own_post') && $aPost['user_id'] == Phpfox::getUserId()) || Phpfox::getUserParam('forum.can_delete_other_posts'))
			{
				$bHasAccess = true;
			}
		}
	...
	public function updateText()
	{
		...
		{		
			if (Phpfox::getService('forum.post.process')->updateText($this->get('edit'), $sTxt, $aVals))
			{
				...
				$aPost['count'] = '';
				$this->template()->assign(array(
						'bIsPostUpdateText' => true,
						'aPost' => $aPost
					)
				)->getTemplate('forum.block.post');

				$this->html('#post' . $aPost['post_id'], $this->getContent(false))->call('$Core.forum.processReply(' . $aPost['post_id'] . ');');	
				$this->call('tb_remove();');			
			}
		}
	}
	...
	public function moderation()
	{
		...
		switch ($this->get('action'))
		{
			case 'approve':
				...
				$this->updateCount();
				$sMessage = 'Thread(s) successfully approved.';
				break;			
			case 'delete':
				...
				$sMessage = 'Thread(s) successfully deleted.';
				break;
		}
	...
	public function postModeration()
	{
		...
		switch ($this->get('action'))
		{
			case 'approve':
				...
				$this->updateCount();
				$sMessage = 'Post(s) successfully approved.';
				break;			
			case 'delete':
				Phpfox::getUserParam('forum.can_delete_other_posts', true);
				foreach ((array) $this->get('item_moderate') as $iId)
				...
				$sMessage = 'Post(s) successfully deleted.';
				break;
		}
