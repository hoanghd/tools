 ...
 * @version 		$Id: post.class.php 3257 2011-10-10 09:34:35Z Miguel_Espinoza $
 */
class Forum_Service_Post_Post extends Phpfox_Service 
{
	...
	public function getPost($iId)
	{
		...
		$aPost = $this->database()->select('fp.*, ' . (Phpfox::getParam('core.allow_html') ? 'fpt.text_parsed' : 'fpt.text') . ' AS text, ' . Phpfox::getUserField() . ', u.joined, u.country_iso, uf.signature_clean as signature, uf.total_post, ft.forum_id, ft.group_id, ft.user_id AS thread_user_id, ft.title AS thread_title')
	...
	{
		return $this->database()->select('fp.*, ft.forum_id, fpt.text, ft.group_id')
			->from(Phpfox::getT('forum_post'), 'fp')
			->join(Phpfox::getT('forum_thread'), 'ft', 'ft.thread_id = fp.thread_id')
			->join(Phpfox::getT('forum_post_text'), 'fpt', 'fpt.post_id = fp.post_id')
			->where('fp.post_id = ' . $iId)
