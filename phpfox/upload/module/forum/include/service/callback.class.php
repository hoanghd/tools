...
class Forum_Service_Callback extends Phpfox_Service 
{
	...
	{
	    $aItem = $this->database()->select('ft.title_url, f.name_url')
		    ->from(Phpfox::getT('forum_thread'),'ft')
		    ->leftjoin(Phpfox::getT('forum'), 'f', 'f.forum_id = ft.thread_id')
		    ->where('ft.thread_id = ' . (int)$aParams['item_id'])
		    ->execute('getSlaveRow');
	    return Phpfox::getLib('url')->makeUrl('forum.'.$aItem['name_url'].'.'.$aItem['title_url']);
	}
	...
	public function addLikePost($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Phpfox::getService('notification.process')->add('forum_post_like', $aRow['thread_id'], $aRow['user_id']);				
		}
	}	
	...
	public function getActivityFeedReply($aItem)
	{
		...
		$aRow = $this->database()->select('fp.post_id, ft.thread_id, ft.title, fp.user_id AS post_user_id, fp.total_like, fpt.text_parsed AS text, fp.time_stamp, ' . Phpfox::getUserField())
			->from(Phpfox::getT('forum_post'), 'fp')			
			->join(Phpfox::getT('forum_thread'), 'ft', 'ft.thread_id = fp.thread_id')
			->join(Phpfox::getT('user'), 'u', 'u.user_id = ft.user_id')
			->join(Phpfox::getT('forum_post_text'), 'fpt', 'fpt.post_id = fp.post_id')
		...
		{
			$sPhrase = Phpfox::getPhrase('feed.replied_on_gender_thread_a_href_link_title_a', array('gender' => Phpfox::getService('user')->gender($aRow['gender'], 1), 'link' => $sLink, 'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'], Phpfox::getParam('notification.total_notification_title_length'), '...')));	
		}
		else 
		{
	...
	{		
		$aRow = $this->database()->select('ft.thread_id, ft.title, ft.user_id, fp.total_like, fpt.text_parsed AS text, ft.time_stamp, l.like_id AS is_liked, fp.post_id')
			->from(Phpfox::getT('forum_thread'), 'ft')
