 ...
 * @version 		$Id: thread.class.php 3257 2011-10-10 09:34:35Z Miguel_Espinoza $
 */
...
{	
	...
	public function get($mConditions = array(), $sOrder = 'ft.time_update DESC', $iPage = '', $iPageSize = '', $bCount = true)
	{
		...
		{						
			...
			foreach ($aThreads as $iKey => $aThread)
			{
				...
				{
					case 'new':
						$sCssPhrase = 'Thread contains new posts.';
						break;
					case 'old':
						$sCssPhrase = 'No new posts.';
						break;
					case 'closed':
						$sCssPhrase = 'Thread is closed.';
						break;
				}
	...
	public function getAnnoucements($iForumId = null, $iGroupId = null)
	{
		...
		{			
			...
			$aAnnouncements[$iKey]['css_class_phrase'] = 'No new posts.';
	...
	public function getThread($aThreadCondition = array(), $mConditions = array(), $sOrder = 'fp.time_stamp ASC', $iPage = '', $iPageSize = '', $sPermaView = null)
	{
		...
		$aThread['posts'] = $this->database()->select('fp.*, ' . (Phpfox::getParam('core.allow_html') ? 'fpt.text_parsed' : 'fpt.text') . ' AS text, ' . Phpfox::getUserField() . ', u.joined, u.country_iso, uf.signature_clean as signature, uf.total_post')
			->from(Phpfox::getT('forum_post'), 'fp')
		...
		foreach ($aThread['posts'] as $iKey => $aPost)
		{
			...
			$aThread['posts'][$iKey]['aFeed'] = array(				
				...
				'report_phrase' => 'Report this post',
				'force_report' => true,
	...
	public function getForRss($iLimit, $sForumIds = null, $iGroupId = 0)
	{
		...
		$aRows = $this->database()->select('ft.title, ft.title_url, ft.forum_id, ft.group_id, ft.time_stamp, ' . (Phpfox::getParam('core.allow_html') ? 'fpt.text_parsed' : 'fpt.text') . ' AS description, f.name AS forum_name, f.name_url AS forum_url, ' . Phpfox::getUserField())
			->from($this->_sTable, 'ft')
			->join(Phpfox::getT('user'), 'u', 'u.user_id = ft.user_id')
			->join(Phpfox::getT('forum_post_text'), 'fpt', 'fpt.post_id = ft.start_id')
			->leftJoin(Phpfox::getT('forum'), 'f', 'f.forum_id = ft.forum_id')
		...
		{
			$aRows[$iKey]['link'] = ($aRow['group_id'] ? Phpfox::getLib('url')->makeUrl('group.forum', array($aRow['title_url'], 'id' => $aRow['group_id'])) : Phpfox::getLib('url')->makeUrl('forum', array($aRow['forum_url'] . '-' . $aRow['forum_id'], $aRow['title_url'])));
