...
class Blog_Service_Callback extends Phpfox_Service 
{
	...
	public function getTopUsers()
	{
		...
		$aBlogs = $this->database()->select('COUNT(b.blog_id) AS top_total, ' . Phpfox::getUserField())
			->from(Phpfox::getT('user'), 'u')
			->join(Phpfox::getT('blog'), 'b', 'b.user_id = u.user_id AND b.is_approved = 1 AND b.privacy = 1 AND b.post_status = 1')
			...
			->execute('getRows', array(
				'cache' => Phpfox::getParam('blog.cache_top_bloggers'),
				'cache_name' => 'user_activity_blog',
				'cache_limit' => Phpfox::getParam('blog.cache_top_bloggers_limit')
			)
		);		
	...
	public function addComment($aVals, $iUserId = null, $sUserName = null)
	{
		...
		$aBlog = $this->database()->select('u.full_name, u.user_id, u.gender, u.user_name, b.title, b.blog_id, b.privacy')
			->from($this->_sTable, 'b')
			->join(Phpfox::getT('user'), 'u', 'u.user_id = b.user_id')
	...
	{
		return '<a href="' . Phpfox::getLib('url')->makeUrl('comment.view', array('id' => $iId)) . '">On ' . $sName . '\'s blog.</a>';
	}	
	...
	public function updateCounter($iId, $iPage, $iPageLimit)
	{
		...
		if ($iId == 'blog-total')
		{
			...
			$aRows = $this->database()->select('u.user_id, u.user_name, u.full_name, COUNT(b.blog_id) AS total_items')
				->from(Phpfox::getT('user'), 'u')
				->leftJoin(Phpfox::getT('blog'), 'b', 'b.user_id = b.user_id AND b.is_approved = 1 AND b.post_status = 1')
