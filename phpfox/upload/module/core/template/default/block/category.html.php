...
<ul {if isset($sUlClass)}class="{$sUlClass}"{else}class="action"{/if}>
	...
	<a href="{$aCategory.url}{if Phpfox::getLib('request')->get('view') != ''}view_{request var='view'}/{/if}" id="{if isset($sModule)}{$sModule}_{/if}category_{$aCategory.category_id}">
	...
	<ul>
		...
		<div id="show_more_{$aCategory.category_id}" class="category_show_more_less" style="text-align:right;vertical-align:middle;"><a href="#" onclick="return false;">{img theme='misc/plus.gif' class='v_middle'}{phrase var='user.view_more'}</a></div>
		<div id="show_less_{$aCategory.category_id}" class="category_show_more_less" style="display:none;text-align:right;vertical-align:middle;"><a href="#" onclick="return false;">{img theme='misc/minus.gif' class='v_middle'}{phrase var='core.view_less'}</a></div>
	    </li>
		{/if}
	</ul>
		{/if}
