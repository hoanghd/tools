...
class User_Service_Group_Process extends Phpfox_Service 
{
	...
	{		
		...
		if (empty($sVar))
		{
			$aGroupSettings = $this->database()->select('ugc.setting_id, ugc.module_id, ugc.name, ugc.default_value')
				->from(Phpfox::getT('user_group_custom'), 'ugc')
				->join(Phpfox::getT('user_group_setting'), 'ugs', 'ugc.setting_id = ugs.setting_id AND ugc.user_group_id = ' . (int) $aVals['inherit_id'])
				->execute('getRows');  				
		}
		else 
		{
