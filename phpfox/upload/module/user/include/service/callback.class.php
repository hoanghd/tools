...
class User_Service_Callback extends Phpfox_Service
{
	...
	public function getApiSupportedMethods()
	{
		...
		$aMethods[] = array(
			...
			'detail' => 'Get information about a user based on the user ID# you pass. If you do not pass the #{USER_ID} we will return information about the user that is currently logged in.',
			'type' => 'GET',			
	...
	public function getNotificationStatus_Like($aNotification)
	{
		...
		{
			$sPhrase = Phpfox::getService('notification')->getUsers($aNotification) . ' liked ' . Phpfox::getService('user')->gender($aRow['gender'], 1) . ' own status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...') . '"';	
		}
		elseif ($aRow['user_id'] == Phpfox::getUserId())		
		{
			$sPhrase = Phpfox::getService('notification')->getUsers($aNotification) . ' liked your status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...') . '"';
		}
		else 
		{
			$sPhrase = Phpfox::getService('notification')->getUsers($aNotification) . ' liked <span class="drop_data_user">' . $aRow['full_name'] . '\'s</span> status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...') . '"';
		}
	...
	{		
		...
		Phpfox::getService('comment.process')->notify(array(
				...
				'owner_subject' => Phpfox::getUserBy('full_name') . ' commented on your status update.',
				'owner_message' => Phpfox::getUserBy('full_name') . " commented on your status update \"" . Phpfox::getLib('parse.output')->shorten($aRow['content'], 50, '...') . "\".\nTo see the comment thread, follow the link below:\n<a href=\"" . $sLink . "\">" . $sLink . "</a>",
				'owner_notification' => 'comment.add_new_comment',
				'notify_id' => 'comment_user_status',
				'mass_id' => 'user_status',
				'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? Phpfox::getUserBy('full_name') . ' commented on ' . Phpfox::getService('user')->gender($aRow['gender'], 1) . ' status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], 10, '...') . '".' : Phpfox::getUserBy('full_name') . ' commented on ' . $aRow['full_name'] . '\'s status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], 150, '...') . '".'),
				'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? Phpfox::getUserBy('full_name') . " commented on " . Phpfox::getService('user')->gender($aRow['gender'], 1) . " status update \"<a href=\"" . $sLink . "\">" . Phpfox::getLib('parse.output')->shorten($aRow['content'], 50, '...') . "</a>\".\nTo see the comment thread, follow the link below:\n<a href=\"" . $sLink . "\">" . $sLink . "</a>" : Phpfox::getUserBy('full_name') . " commented on " . $aRow['full_name'] . "'s status update \"<a href=\"" . $sLink . "\">" . Phpfox::getLib('parse.output')->shorten($aRow['content'], 50, '...') . "</a>\".\nTo see the comment thread, follow the link below:\n<a href=\"" . $sLink . "\">" . $sLink . "</a>")
			)
		);		
	}		
	...
	public function getCommentNotificationStatus($aNotification)
	{
		...
		if ($aNotification['user_id'] == $aRow['user_id'])
		{
			$sPhrase = '<span class="drop_data_user">' . $aNotification['full_name'] . '</span> commented on ' . Phpfox::getService('user')->gender($aRow['gender'], 1) . ' status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...') . '"';	
		}
		elseif ($aRow['user_id'] == Phpfox::getUserId())		
		{
			$sPhrase = '<span class="drop_data_user">' . $aNotification['full_name'] . '</span> commented on your status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...') . '"';
		}
		else 
		{
			$sPhrase = '<span class="drop_data_user">' . $aNotification['full_name'] . '</span> commented on <span class="drop_data_user">' . $aRow['full_name'] . '\'s</span> status update "' . Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...') . '"';
		}
