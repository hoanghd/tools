...
class Event_Component_Controller_Index extends Phpfox_Component
{
	...
	{		
		...
		{
			if (Phpfox::getService('event.process')->delete($iDeleteId))
			{
		...
		switch ($sView)
		{
			...
			default:
				if ($bIsUserProfile)
				{
					$this->search()->setCondition('AND m.view_id ' . ($aUser['user_id'] == Phpfox::getUserId() ? 'IN(0,2)' : '= 0') . ' AND m.privacy IN(' . (Phpfox::getParam('core.section_privacy_item_browsing') ? '%PRIVACY%' : Phpfox::getService('privacy')->getForBrowse($aUser)) . ') AND m.user_id = ' . (int) $aUser['user_id']);
				}
