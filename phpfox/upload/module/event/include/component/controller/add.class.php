...
class Event_Component_Controller_Add extends Phpfox_Component
{
	...
	public function process()
	{
			...
			{				
				if ($bIsEdit)
				{
					if (Phpfox::getService('event.process')->update($aEvent['event_id'], $aVals, $aEvent))
					{
						...
						{
							case 'customize':
								$this->url()->send('event.add.invite.setup', array('id' => $aEvent['event_id']), 'Successfully added a photo to your event.');	
								break;
							case 'invite':
								...
								break;
							default:
								$this->url()->send('event.add', array('id' => $aEvent['event_id']), 'Event successfully updated.');	
								break;
						}	
					}
					else
		...
		$this->template()->setTitle(($bIsEdit ? Phpfox::getPhrase('event.managing_event') . ': ' . $aEvent['title'] : 'Create an Event'))
			->setFullSite()			
