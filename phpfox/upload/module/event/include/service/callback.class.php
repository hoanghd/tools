...
class Event_Service_Callback extends Phpfox_Service 
{
	...
	{
		return '<a href="' . Phpfox::getLib('url')->makeUrl('comment.view', array('id' => $iId)) . '">On ' . $sName . '\'s event.</a>';
	}	
	...
	public function getNotificationLike($aNotification)
	{
		...
		$sUsers = Phpfox::getService('notification')->getUsers($aNotification);
		$sTitle = Phpfox::getService('user')->gender($aRow['gender'], 1) . ' own event "' . Phpfox::getLib('parse.output')->shorten($aRow['title'], Phpfox::getParam('notification.total_notification_title_length'), '...');
	...
	public function getProfileMenu($aUser)
	{
		...
		$aMenus[] = array(
			...
			'url' => 'profile.event',
			'total' => (int) (isset($aUser['total_event']) ? $aUser['total_event'] : 0),
	...
	public function getTotalItemCount($iUserId)
	{
		return array(
			'field' => 'total_event',
			'total' => $this->database()->select('COUNT(*)')->from(Phpfox::getT('event'))->where('view_id = 0 AND user_id = ' . (int) $iUserId)->execute('getSlaveField')
		);	
	}	
