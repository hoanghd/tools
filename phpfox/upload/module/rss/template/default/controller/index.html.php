 ...
 * @author  		Raymond Benc
 * @package  		Module_Rss
 * @version 		$Id: index.html.php 1494 2010-03-05 14:23:08Z Raymond_Benc $
 */
	...
	{foreach from=$aFeeds item=aFeed}
		...
		<li><div class="parent">{phrase var=$aFeed.title_var}</div>
			...
			{foreach from=$aFeed.child key=sLink item=sPhrase}
				<li><a href="{$sLink}">{img theme='rss/tiny.png' class='v_middle'} {$sPhrase|convert}</a></li>	
			{/foreach}
			</ul>
		</li>
		{else}
		<li><a href="{url link='rss' id=$aFeed.feed_id}">{img theme='rss/tiny.png' class='v_middle'} {phrase var=$aFeed.title_var}</a></li>
		{/if}
	{/foreach}
	</ul>
</div>
