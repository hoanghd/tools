...
class Photo_Component_Ajax_Ajax extends Phpfox_Ajax
{
    ...
    public function feature()
    {
		...
		if (Phpfox::getService('photo.process')->feature($this->get('photo_id'), $this->get('type')))
		{
		    if ($this->get('type') == '1')
		    {
				$sHtml = '<a href="#" title="' . Phpfox::getPhrase('photo.un_feature_this_photo') . '" onclick="$.ajaxCall(\'photo.feature\', \'photo_id=' . $this->get('photo_id') . '&amp;type=0\'); return false;">Un-Feature</a>';
		    }
		    else
		    {
				$sHtml = '<a href="#" title="' . Phpfox::getPhrase('photo.feature_this_photo') . '" onclick="$.ajaxCall(\'photo.feature\', \'photo_id=' . $this->get('photo_id') . '&amp;type=1\'); return false;">Feature</a>';
		    }
		...
		if ($iNotCompleted === 0)
		{
		    ...
		    {				
			    ...
			    if ($this->get('action') == 'view_photo')
			    {
					...
					$this->call('window.parent.location.href = \'' . Phpfox::getLib('url')->permalink('photo', $aPhoto['photo_id'], $aPhoto['title']) . '\';');
			    }
			    ...
			    else
			    {
					...
					$this->call('window.location.href = \'' . Phpfox::getLib('url')->permalink('photo', $aPhoto['photo_id'], $aPhoto['title']) . '\';');
			    }
	...
	public function moderation()
	{
		...
		switch ($this->get('action'))
		{
			case 'approve':
				...
				$sMessage = 'Photo(s) successfully approved.';
				break;			
			case 'delete':
				...
				$sMessage = 'Photo(s) successfully deleted.';
				break;
		}
	...
	public function makeProfilePicture()
	{
		...
		Phpfox::addMessage('Profile photo successfully updated.');
