...
class Admincp_Service_Callback extends Phpfox_Service
{
	...
	{				
		if ($iId == 'import-groups')
		{
			...
			$aGroupType = $this->database()->select('pc.*')
			...
			$iCnt = $this->database()->select('COUNT(*)')
				...
				$aGroups = $this->database()->select('g.*, gt.*')
				...
				{					
					$iPageId = $this->database()->insert(Phpfox::getT('pages'), array(
							'view_id' => '0',
							'type_id' => $aGroupType['type_id'],
							'category_id' => $aGroupType['category_id'],
		...
		foreach ($aRows as $aRow)
		{
			...
			if (Phpfox::isModule('bulletin'))
			{
				$aBulletins = $this->database()->select('b.*, bt.*')
					...
					->join(Phpfox::getT('bulletin_text'), 'bt', 'bt.bulletin_id = bt.bulletin_id')
