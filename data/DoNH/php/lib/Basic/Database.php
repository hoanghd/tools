...
class Database
{
     ...
     * @var array
     */
    private $aMessage = array('db.common'  	 => 'Database error',
     ...
     */
    public function escapeLike($sParam)
    {
        return str_replace(array('%','_'), array('\%','\_'), $this->escape($sParam));
    }
     ...
     */
    public function unescape($mParam)
    {
        if (is_array($mParam))
            return array_map(array($this, 'unescape'), $mParam);
     ...
     */
    public function getRows($sSql, $bAssoc = true, $sKeyField='')
    {
        $aRows = array();
     ...
     * @see fetchRow()
     */
    public function fetchRows($hRes, $bAssoc=true, $sKeyField = '')
    {
        $aRows = array();
        $bAssoc = ($bAssoc ? MYSQL_ASSOC : MYSQL_NUM);
        while($aRow = mysql_fetch_array($hRes, $bAssoc)) {
     ...
     */
    public function selectField($sTable, $sCol, $sCond) {
     ...
     */
    public function _addError($sKey, $mParam=array())
    {
